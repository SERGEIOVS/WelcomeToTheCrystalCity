...
from Items import items_max,item
from Vehicles import vehicle_sit
pg.init()
...
"step_sound"        : pg.mixer.Sound('Audio/sounds/steps/grass/'       + str(step_sound_num)   + '.mp3'),
"rifle_sound"       :pg.mixer.Sound('Audio/sounds/firegun/automatic/'  + str(rifle_sound_num)  + '.mp3'),
"spawn_sound"       : pg.mixer.Sound('Audio/sounds/spawn/'             + str(spawn_sound_num)  + '.mp3'),
"scream_sound"      : pg.mixer.Sound('Audio/sounds/screams/far/'       + str(scream_sound_num) + '.mp3'),
"death_sound"       : pg.mixer.Sound('Audio/sounds/death/'             + str(death_sound_num)  + '.mp3'),
"pickup_sound"      : pg.mixer.Sound('Audio/sounds/pickup/'            + str(pickup_sound_num) + '.mp3'),
"enemy_sound"       : pg.mixer.Sound('Audio/sounds/roar/'              + str(enemy_sound_num)  + '.mp3'),
"click_sound"       : pg.mixer.Sound('Audio/sounds/clicks/'            + str(click_sound_num)  + '.mp3'),
"welcome"           : pg.mixer.Sound('Audio/speech/langs/'             + str(language)         + '/welcome/' + str(int(welcome_num)) + '.mp3'),
"inv_is_full_sound" : pg.mixer.Sound('Audio/sounds/massages/Full inventory/' + str(inv_full_sound_num) +'.mp3')
...
"health_icon"    : pg.image.load('Interface/Icons/Health/'          + str(vars1["health_icon_num"])    + '.png'),
"ammo_icon"      : pg.image.load('Interface/Icons/Ammo/'            + str(vars1["ammo_icon_num"])      + '.png'), 
"armor_icon"     : pg.image.load('Interface/Icons/Armor/'           + str(vars1["armor_icon_num"])     + '.png'),
...
"cancel_icon_x"  :  int(screen_width) - 50,
"cancel_icon_y" : 50
...
#chat_moderation() - функция вызывается при проверке чата на запрещенку
...
[-camera1.rect[0] + 1000 + int(buildings_file1[i].split(',')[0]) + meter * 7  * math.cos(fuel) , -camera1.rect[1] + 1000 + int(buildings_file1[i].split(',')[1]) ]
        ...
        if show_interface == 1:
           ...
           # pg.draw.line(screen   , (0   , 0 , 0 ) ,    [ -camera.rect[0] + int(camera_x) + int(screen_width) / 2 + hero_checkpoint_offset_x , -camera.rect[ 1 ] + int(camera_y) + int(screen_height) / 2 + 100 + hero_checkpoint_offset_y ] , [ -camera.rect[ 0 ] + int(checkpoints_file1[0].split(',')[0]) , -camera.rect[ 1 ] + int(checkpoints_file1[0].split(',')[1])] , 1 )
            ...
            screen.blit(vars1["show_hero_armor"] , ( bigfont + 10 , int(screen_height ) - bigfont * 5))     , ( 10 , int(screen_height ) - bigfont * 5))
            screen.blit(vars1["show_ammo"]       , ( bigfont + 10 , int(screen_height ) - bigfont * 4))
            screen.blit(vars1["show_health"]     , ( bigfont + 10 , int(screen_height ) - bigfont * 6))
            screen.blit(vars1["show_radiation"]  , ( bigfont + 10 , int(screen_height ) - bigfont * 3))
            screen.blit(vars1["show_energy"]     , ( bigfont + 10 , int(screen_height ) - bigfont * 2))
            screen.blit(vars1["show_money"]      , ( 10           , int(screen_height ) - bigfont))



            screen.blit( icons_dict['pics']['ammo_icon']      , ( icons_dict['cancel_icon_x'] , icons_dict['cancel_icon_y']))
            screen.blit(icons_dict['pics']["energy_icon"]     , ( 10 , int(screen_height ) - bigfont * 2))
            screen.blit(icons_dict['pics']["radiation_icon"]  , ( 10 , int(screen_height ) - bigfont * 3))
            screen.blit(icons_dict['pics']["ammo_icon"]       , ( 10 , int(screen_height ) - bigfont * 4))
            screen.blit(icons_dict['pics']["armor_icon"]      , ( 10 , int(screen_height ) - bigfont * 5))
            screen.blit(icons_dict['pics']['health'])         , ( 10 , int(screen_height ) - bigfont * 6)
